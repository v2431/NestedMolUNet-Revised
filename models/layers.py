...
class MESPool(torch.nn.Module):
    ...
    def forward(self, graph, batch_size=None,
        ...
        # ===== 🔴 专业修复：安全的余弦相似度计算 =====
        eps = 1e-8
        q_norm = Q.norm(p=2, dim=-1).clamp_min(eps)
        k_norm = K.norm(p=2, dim=-1).clamp_min(eps)
        denominator = q_norm * k_norm
        numerator = (K * Q).sum(-1)
        score = numerator / denominator
        # 加保险，避免边界问题
        score = score.clamp(-1.0 + 1e-6, 1.0 - 1e-6)
...
class MPBlock(nn.Module):
    def __init__(self, in_dim, config, num_layer=None):
        ...
        params = {'in_channels': in_dim,
                  ...
                  'jk': mp_jk}
        if mp_method == 'PNA':
            # ===== 专业修复：保留 mean+max，移除 std（std 最容易出数值问题）=====
            params['aggregators'] = ['mean', 'max']  # 保留 mean 和 max，比带 std 稳
            params['scalers'] = ['identity']  # 只保留 identity，避免缩放导致数值不稳定
