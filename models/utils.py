...
def get_deg_from_list(data_list):
    ...
    # ===== 关键修改：强制在 CPU 上创建度分布张量 =====
    deg = torch.zeros(max_degree + 1, dtype=torch.long, device='cpu')
    ...
    for data in data_list:
        ...
        d = degree(edge_index_cpu[1], num_nodes=data.num_nodes, dtype=torch.long)
        # 计算 bincount 并累加到 deg
